...
This code was written in collaboration with my skillful classmate Thibaut Eloy from IMT Atlantique, France.
...
structures (like the RBF networks). Also, we found it intersting to share it with anyone who would be
...
def test_func(x):
    return [5*(x[0]-1/5)*(x[0]-1/2)*(x[0]-2/3)]
...
def procedure(startingEtha, stepEtha, numberEthas, plot = False) :
    ...
    neuNetCand.train2(test_func, trainingSet, validationSet, 1, 30, 30, 1.01)
    neuNetCand.train2(test_func, trainingSet, validationSet, 1, 60, 60, 1.02)
